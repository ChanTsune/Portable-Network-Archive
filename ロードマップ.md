...
| -r / --append | 既存アーカイブに追加 | ⚠️ `--append` のみ。`-r` 未割り当て | `cli/src/command/stdio.rs:67` で長オプション提供。`-r` alias追加と、ファイルベース限定のbsdtar挙動（圧縮不可）に合わせた制約チェックを追加する。 |
| -t / --list | アーカイブ一覧表示 | ✅ | `cli/src/command/stdio.rs:65`。`-v` 指定時のbsd形式出力を `ListOptions::format` で制御する必要あり。 |
| -u / --update | 新しい場合のみ更新 | ❌ | `command::update::UpdateCommand` は存在するが stdio から未露出。新しい分岐と短縮フラグを追加する。 |
| -x / --extract / --get | アーカイブ展開 | ✅ | `cli/src/command/stdio.rs:63`。`--get` alias 追加、`-O` 連携のストリーム抽出対応が課題。 |
| -d / --delete | アーカイブから削除 | ❌ | `command::delete::DeleteCommand` は存在。stdio サブコマンドにブリッジを追加。 |
| -A / --append-to | 他アーカイブ連結 | ❌ | `concat` コマンドと統合する設計が必要。 |
| @archive | 入力アーカイブ内容を連結 | ❌ | パス先頭 `@` を検出し `collect_split_archives` へ委譲する仕組みを追加。 |

### 1.2 入出力・探索系
| bsdtarオプション | 要約 | 現状 | 実装・課題メモ |
| --- | --- | --- | --- |
| -a / --auto-compress | 拡張子で圧縮推定 | ❌ | 自動推定ロジックと外部圧縮器マッピングを追加。 |
| --acls | ACL を保存/復元 | ⚠️ `--keep-acl` 長オプションのみ・`--unstable` 要求 | `cli/src/command/stdio.rs:110`。名称 alias と `--no-acls` 対応が必要。 |
| -B / --read-full-blocks | 完全ブロック読み込み | ❌ | テープI/O未実装。サポート対象外ならドキュメント化。 |
| -b / --block-size | ブロックサイズ指定 | ❌ | テープサポート検討。 |
| -C / --cd / --directory | 作業ディレクトリ変更 | ✅ | `cli/src/command/stdio.rs:223` (`-C` 対応)。抽出時ディレクトリ移動の挙動差を確認。 |
| --chroot | chroot実行 | ❌ | 権限周り要検討。 |
| --clear-nochange-fflags | macOS fflags操作 | ❌ | 対応不要なら非互換として明記。 |
...
| --exclude-vcs | VCSファイル除外 | ⚠️ `--unstable` | `cli/src/command/stdio.rs:135`。安定化。 |
| --fflags | BSD file flags保持 | ❌ | PNAコアに機能追加が必要。 |
| --format format | フォーマット指定 | ⚠️ 受理のみ | Clap で受け付けて互換メッセージを出しつつ PNA 固有フォーマットへマッピング。将来的に複数フォーマット対応する場合の拡張ポイントを設計。 |
...
| -h | `-L` の別名 | ❌ | `follow_links` に短縮 alias 追加。 |
| -I | `-T` の別名 | ❌ | Clap alias追加。 |
| --help | ヘルプ | ✅ | Clap 自動生成。bsdtar互換の例示差分を表に反映させる。 |
| -k / --keep-old-files | 既存ファイルを保護 | ❌ | `OutputOption` に上書き抑止フラグを追加し、bsdtarと同じ既存ファイル保持を実装。 |
| --keep-newer-files | 既存の方が新しければ保持 | ❌ | 上記と同じく `OutputOption` 拡張とタイムスタンプ比較を追加。 |
| -L / --dereference | シンボリックリンクを辿る | ⚠️ `--follow-links` 長オプションのみ (`--unstable`) | `cli/src/command/stdio.rs:139`。`-L` alias追加。 |
| -l / --check-links | ハードリンク参照数チェック | ❌ | `collect_items` でリンクカウント追跡し、欠落時にエラーを出す処理を追加。 |
...
| --ignore-zeros | ゼロブロック無視 | ❌ | リーダ拡張が必要。 |
...
| --one-file-system | 同一ファイルシステムのみ | ❌ | `collect_items` に `st_dev` 判定を追加。 |
| --nodump | nodump 属性尊重 | ❌ | Filesystemメタデータ処理に対応が必要。 |
| --options key=val | フォーマット依存設定 | ❌ | Clapで key=value パース追加 & PNA coreへ橋渡し。 |
| --use-compress-program prog | 外部圧縮器 | ❌ | プロセス起動と stdio パイプ実装が必要。 |
...
| --newer date | 指定日時より新しいもののみ | ❌ | `UpdateCommand` に該当ロジックあり。stdio 側で expose。 |
| --newer-mtime date | mtime基準 | ❌ | 同上。 |
| --newer-than file | 指定ファイルより新しい | ❌ | 同上。 |
| --newer-mtime-than file | mtime基準ファイル | ❌ | 同上。 |
| --older date | 指定日時より古いもののみ | ❌ | `UpdateCommand` の older_* を活用。 |
| --older-mtime date | mtime基準 | ❌ | 同上。 |
| --older-than file | ファイル比較 | ❌ | 同上。 |
| --older-mtime-than file | mtime比較 | ❌ | 同上。 |
| --strip-components count | パス先頭除去 | ✅ | `cli/src/command/stdio.rs:149`。互換テスト未整備（追加）。 |
| -s pattern | BSD tar 互換のリネーム | ⚠️ `--unstable` 必須 | `cli/src/command/stdio.rs:203`。正規表現互換性の検証と `--transform` との相互作用テストが必要。 |
...
| bsdtarオプション | 要約 | 現状 | 実装・課題メモ |
| --- | --- | --- | --- |
| -p / --preserve-permissions | 権限保持 | ⚠️ `--keep-permission` 長オプション (`--unstable`) | `cli/src/command/stdio.rs:99`。Alias追加と extract時挙動確認。 |
| -m / --modification-time | 抽出時にmtimeを現時刻へ | ❌ | `OutputOption` へフラグを追加。 |
...
| --ignore-zeros | 上掲 | ❌ | `Archive` リーダにサポート追加。 |
