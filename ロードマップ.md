...
| --exclude-vcs | VCSファイル除外 | ✅ | `cli/src/command/stdio.rs:627` ほか。list からも `--unstable` 依存を撤廃済み。 |
...
| -h | `-L` の別名 | ✅ | `cli/src/command/stdio.rs:312` に `visible_short_alias = 'h'` を追加済み。 |
...
| -L / --dereference | シンボリックリンクを辿る | ✅ | `cli/src/command/stdio.rs:312` ほかで `short = 'L'` を追加。 |
