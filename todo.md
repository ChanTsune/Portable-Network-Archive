...
---
## 0. 共通準備
- [ ] 0-1 `bsdtar` 利用可否確認 (`bsdtar --version` を macOS/Linux で実行)。存在しない場合は libarchive をインストール。
- [ ] 0-2 開発環境基準整備: `cargo fmt --check`, `cargo clippy --all-targets`, `cargo test --all` を実行し、ベースラインを保存。
