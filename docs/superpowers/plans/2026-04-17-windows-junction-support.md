...
Route the target through `PathnameEditor::edit_junction` (the method added in Task 2.1). It applies user-specified `-s` / `--transform` substitutions and builds an `EntryReference` via the shared helper that `edit_symlink` also uses, so junction and symlink targets behave consistently under user transforms. Invariant I1 makes the helper's internal `to_string_lossy` conversion effectively lossless for real inputs.
...
fn extract_junction_does_not_mutate_external_target() {
    ...
    // `restore_metadata`'s chmod branch is skipped when
    // `item.metadata().permission()` is None, so a junction without a
    // Permission chunk slips through even if the Task 4.1 early-return is
    // removed. With mode 0o755 stamped, a regression `chmod(link, 0o755)`
    // follows the link and changes the external target's mode, firing the
    // `baseline_perms != after_perms` assertion below.
    let archive_path = format!("{base}/{base}.pna");
    fs::write(
        &archive_path,
