...
#
# Test mapping to C original:
#   Bats Test 1 = C test1 dir (-x -T before -r append)
#   Bats Test 2 = C test3 dir (-x without -T, verifies -r append)
#   Bats Test 3 = C test2 dir (-x -T after -r append)
#
# NOTE: The C original's gnarlyFilesSupported feature (file with embedded
# newline in its name: "d1/d2/f\x0a") is omitted from this port.
# Handling filenames with embedded newlines requires platform-specific
# conditional logic that is fragile in shell tests.
#
# NOTE: The C original's test4 section (scaffolding for -C within filelist)
# is omitted — the original had only a TODO and no assertions.
...
setup_file() {
  ...
  # Save archive before -r append (Test 1 extracts from this state).
  cp test1.tar test_pre_append.tar
...
# Test 1: Use -x -T to dearchive only the files in the filelist (before -r)
@test "Test 1: -x -T to dearchive the filelist entries" {
  run bash -c "$EXECUTABLE -x -f ../test_pre_append.tar -T ../filelist -C . >test.out 2>test.err"
  assert_success
