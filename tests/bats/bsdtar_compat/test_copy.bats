...
setup_file() {
  ...
  mkdir -p original/f original/l original/m original/s original/d

  for ((i=1; i<LOOP_MAX; i++)); do
    fname=$(generate_filename "$i")

    # File
    echo -n "f/$fname" > "original/f/$fname"

    # Hardlinks
    ln "original/f/$fname" "original/l/$fname"
    ln "original/f/$fname" "original/m/$fname"

    # Symlink
    ln -s "../f/$fname" "original/s/$fname"

    # Directory
    mkdir "original/d/$fname"
  done
  ...
  # Create archive with --format pax
  run bash -c "$EXECUTABLE -c -f archive_pax --format pax -C original f d l m s >pack_pax.out 2>pack_pax.err"
  assert_success

  # Extract pax
  mkdir -p extracted_pax
  run bash -c "$EXECUTABLE -x -f archive_pax -C extracted_pax >unpack_pax.out 2>unpack_pax.err"
  assert_success
  assert_file_empty unpack_pax.err
  assert_file_empty unpack_pax.out
}
...
# Test 5: Same with pax format - verify files.
@test "Test 5: Files roundtrip with pax format" {
  cd "$BATS_FILE_TMPDIR"
  local errors=0
  for ((i=1; i<LOOP_MAX; i++)); do
    fname=$(generate_filename "$i")
    if [ ! -f "extracted_pax/f/$fname" ]; then
      echo "MISSING: f/$fname" >&2
      errors=$((errors + 1))
    fi
  done
