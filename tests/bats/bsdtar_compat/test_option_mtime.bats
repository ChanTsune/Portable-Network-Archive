...
EXECUTABLE_KEEP_TS="pna experimental stdio --unstable --keep-dir --overwrite --keep-timestamp"
...
@test "Test 1: --mtime without --clamp-mtime" {
  run bash -c "$EXECUTABLE_KEEP_TS -xf ../noclamp.tar >x.out 2>x.err"
  assert_success
  assert_equal "$(stat -c %Y new_mtime)" "86400"
  assert_equal "$(stat -c %Y mid_mtime)" "86400"
  assert_equal "$(stat -c %Y old_mtime)" "86400"
}
...
@test "Test 2: --mtime with --clamp-mtime" {
  run bash -c "$EXECUTABLE_KEEP_TS -xf ../clamp.tar >x.out 2>x.err"
  assert_success
  assert_equal "$(stat -c %Y new_mtime)" "86400"
  assert_equal "$(stat -c %Y mid_mtime)" "10000"
  assert_equal "$(stat -c %Y old_mtime)" "1"
}

# Test 3: Archive-to-archive copy with --mtime 0
@test "Test 3: Archive-to-archive copy with --mtime 0" {
  run bash -c "$EXECUTABLE_KEEP_TS -xf ../archive2archive.tar >x.out 2>x.err"
  assert_success
  assert_equal "$(stat -c %Y new_mtime)" "0"
  assert_equal "$(stat -c %Y mid_mtime)" "0"
  assert_equal "$(stat -c %Y old_mtime)" "0"
}
