...
# Ported from: libarchive tar/test/test_option_newer_than.c
...
setup_file() {
  ...
  run bash -c "cd test1in && $EXECUTABLE --format pax -cf ../test1.tar --newer-than middle.txt old.txt middle.txt new.txt a"
  assert_success
...
# Test 1: Extract test1.tar to verify what got archived with --newer-than on create.
@test "Test 1: --newer-than on create filters old files" {
  run bash -c "$EXECUTABLE -x -f ../test1.tar >x.out 2>x.err"
  assert_success
  assert_file_exist new.txt
  assert_file_exist a/b/new.txt
  assert_file_not_exist middle.txt
  assert_file_not_exist old.txt
}

# Test 2: Extract test2.tar with --newer-than on extract.
@test "Test 2: --newer-than on extract filters old entries" {
  run bash -c "$EXECUTABLE -x -f ../test2.tar --newer-than ../test1in/middle.txt >x.out 2>x.err"
  assert_success
  assert_file_exist new.txt
  assert_file_exist a/b/new.txt
  assert_file_not_exist middle.txt
