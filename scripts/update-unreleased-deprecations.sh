...
set +e
rg -l -0 --glob '*.rs' --fixed-strings 'since = "unreleased"' \
  "${search_roots[@]}" > "$matches_file"
rg_status=$?
set -e

if [[ $rg_status -gt 1 ]]; then
  echo "Failed to search for unreleased deprecation markers" >&2
  exit "$rg_status"
fi

if [[ ! -s "$matches_file" ]]; then
  echo "No unreleased deprecation markers found under $crate_root"
  exit 0
fi
...
while IFS= read -r -d '' file; do
  ...
  perl -0pi -e '
    ...
    s/\Qsince = "unreleased"\E/since = "$version"/g;
  ' "$file"
done < "$matches_file"
...
set +e
rg --glob '*.rs' --fixed-strings 'since = "unreleased"' "${search_roots[@]}" >/dev/null
remaining_status=$?
set -e

if [[ $remaining_status -eq 0 ]]; then
  echo "Unreleased deprecation markers remain under $crate_root" >&2
  exit 1
fi
