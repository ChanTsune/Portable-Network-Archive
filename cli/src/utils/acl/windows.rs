...
pub fn set_acl(path: &Path, acl: Vec<chunk::Ace>) -> io::Result<()> {
    ...
    let mut current_acl = windows_acl::acl::ACL::from_file_path(&path.to_string_lossy(), false).map_err(io::Error::other)?;
...
pub fn get_acl(path: &Path) -> io::Result<Vec<chunk::Ace>> {
    let acl = windows_acl::acl::ACL::from_file_path(&path.to_string_lossy(), false).map_err(io::Error::other)?;
