...
impl PathnameEditor {
    ...
    pub(crate) fn edit_entry_name(&self, path: &Path, absolute_paths: bool) -> Option<EntryName> {
        ...
        if absolute_paths {
            Some(EntryName::from_path_lossy_preserve_root(stripped.as_ref()))
        } else {
    ...
    pub(crate) fn edit_hardlink(
        ...
        let stripped = strip_components(&transformed, self.strip_components)?;
        if absolute_paths {
            Some(EntryReference::from_path_lossy_preserve_root(
                stripped.as_ref(),
            ))
        } else {
    ...
    pub(crate) fn edit_symlink(&self, target: &Path, absolute_paths: bool) -> EntryReference {
        ...
        if let Some(t) = &self.transformers {
            let applied = t.apply(target.to_string_lossy(), true, false);
            if absolute_paths {
                EntryReference::from_utf8_preserve_root(&applied)
            } else {
                EntryReference::from(applied)
            }
        } else {
            if absolute_paths {
