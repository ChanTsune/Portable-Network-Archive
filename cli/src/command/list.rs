...
fn print_entries<'a>(
    ...
    let entries = entries
        .into_iter()
        .filter(|r| {
            let matched = globs.is_empty() || globs.matches_any(r.entry_type.name());
            let time_ok = if options.time_filters.is_active() {
                options.time_filters.matches(r.created, r.modified)
            } else {
                true
            };
