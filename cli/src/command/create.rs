use crate::{
    ...
    command::{
        ...
        core::{
            ...
            read_archive_source, read_paths, read_paths_stdin, write_split_archive,
        },
    },
...
fn create_archive_with_split(
    ...
    if solid {
        ...
        let entries = rx.into_iter().flat_map(|result| match result {
            EntryResult::Single(entry) => vec![entry],
            EntryResult::Batch(entries) => match entries {
                Ok(entries) => entries,
                Err(e) => vec![Err(e)],
            },
        });
