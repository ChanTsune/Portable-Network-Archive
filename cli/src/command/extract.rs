    ...
    Provider: FnMut() -> Option<&'p [u8]> + Send,
{
    ...
    rayon::scope_fifo(|s| -> anyhow::Result<()> {
        run_process_archive(reader, password_provider, |entry| {
            ...
            let Some(item_path) = args.pathname_editor.edit_entry_name(item_path) else {
                return Ok(());
            };

            let path_lock = args.path_locks.get(item_path.as_ref());
            let guard = path_lock.lock_arc();

            let result = process_entry(item, &item_path, password, &args, guard)?;
    ...
    for item in link_entries {
        ...
        let path_lock = args.path_locks.get(item_path.as_ref());
        let guard = path_lock.lock_arc();
        process_entry(item, &item_path, password, &args, guard)
    ...
    pna::RawChunk<T>: Chunk,
{
    ...
    let path = build_output_path(out_dir.as_deref(), name.as_path());
