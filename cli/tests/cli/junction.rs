...
/// given mode. Used by the I2 security fence: extract with `--keep-permission`
/// would fire mode restoration — which, under a regression that drops the
/// junction arm's early return, would follow the link and mutate the external
/// target's mode.
...
fn extract_junction_does_not_mutate_external_target() {
    ...
    // Stamp a permission-mode facet with a mode DIFFERENT from the external
    // target's pre-set mode (0o700). Without this the fence is decorative:
    // extract code's chmod branch is skipped when `metadata().permission_mode()`
