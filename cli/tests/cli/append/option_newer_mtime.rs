...
fn init_archive(path: impl AsRef<std::path::Path>) {
    ...
    fs::rename(
        path.join("empty.pna"),
        path.join("append_with_newer_mtime.pna"),
    )
    .unwrap();
}
...
/// Action: Run `pna append` with `--newer-mtime 2025-10-11T00:00:00Z` to append into the archive.
/// Expectation: The archive contains only entries whose modification times are newer than
///              `2025-10-11T00:00:00Z`.
#[test]
fn append_with_newer_mtime() {
    setup();
    init_archive("append_with_newer_mtime/");
    init_sources("append_with_newer_mtime/in/");
    ...
    let required_entries = ["append_with_newer_mtime/in/keep.txt"];
