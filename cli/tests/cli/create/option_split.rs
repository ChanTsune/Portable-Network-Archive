...
/// Precondition: The input files fit within a single part of the requested split size.
/// Action: Run `pna create --solid` with `--split`.
/// Expectation: The solid archive is written to the requested path with no part-numbered
/// file left behind, and its entries round-trip.
#[test]
fn create_with_solid_split_fitting_in_single_part() {
    setup();
    if Path::new("create_solid_split_single_part").exists() {
        fs::remove_dir_all("create_solid_split_single_part").unwrap();
    }
    fs::create_dir_all("create_solid_split_single_part/in/").unwrap();
    fs::write("create_solid_split_single_part/in/first.txt", b"first").unwrap();
    fs::write("create_solid_split_single_part/in/second.txt", b"second").unwrap();
    ...
    cli::Cli::try_parse_from([
        ...
        "c",
        "-f",
        "create_solid_split_single_part/archive.pna",
        "--overwrite",
        "--solid",
        "--unstable",
        "--split",
        "100kb",
        "create_solid_split_single_part/in/first.txt",
        "create_solid_split_single_part/in/second.txt",
    ])
    .unwrap()
    .execute()
    .unwrap();
    ...
    assert!(
        Path::new("create_solid_split_single_part/archive.pna").exists(),
        "a single-part solid split should be written to the requested path"
    );
    assert!(
        !Path::new("create_solid_split_single_part/archive.part1.pna").exists(),
        "a single-part solid split should not leave a part-numbered file behind"
    );

    let mut seen = HashSet::new();
    archive::for_each_entry("create_solid_split_single_part/archive.pna", |entry| {
        let path = entry.header().path().to_string();
        let mut contents = Vec::new();
        entry
            .reader(pna::ReadOptions::with_password::<&[u8]>(None))
    ...
    assert_eq!(
        ...
        HashSet::from([
            (
                "create_solid_split_single_part/in/first.txt".to_string(),
                b"first".to_vec()
            ),
            (
                "create_solid_split_single_part/in/second.txt".to_string(),
                b"second".to_vec()
            ),
        ])
    );
