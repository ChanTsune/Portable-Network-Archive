...
/// Action: Run `pna experimental chmod` over every entry.
/// Expectation: The requested mode is emitted as fMOd, and the ownership
/// rescued from fPRM is preserved.
#[test]
fn archive_chmod_rewrites_legacy_fprm_archive() {
    setup();
    TestResources::extract_in("0.33.0/zstd_keep_all.pna", "chmod_legacy_fprm/").unwrap();
    let path = "chmod_legacy_fprm/0.33.0/zstd_keep_all.pna";
    ...
    let mut args = vec!["pna", "--quiet", "experimental", "chmod", "-f", path, "644"];
    ...
    archive::for_each_entry(path, |entry| {
        ...
        let expected = pre
            .get(&entry_path)
            .unwrap_or_else(|| panic!("unexpected entry after chmod: {entry_path}"));
        assert_eq!(
        ...
        assert_eq!(
            ...
            *expected,
            "ownership {entry_path}"
        );
    })
    .unwrap();
    assert_eq!(count, pre.len(), "chmod should preserve all entries");
}
