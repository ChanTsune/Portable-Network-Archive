...
pub trait PartProvider<R: Read> {
    ...
    /// suffix conventionally given to the same part on disk.
    ///
...
/// Any closure of the same shape is a provider, mirroring the `next_part`
/// closure taken by [`Archive::write_split_header`].
...
fn cursor_state_error(state: CursorState) -> io::Error {
    let message = match state {
        CursorState::Poisoned => "a streaming entry was dropped before completion",
        CursorState::Finished => "the streaming archive has already finished",
        CursorState::Failed => "the streaming archive is in a failed state",
        CursorState::Ready => "the streaming archive is ready",
    };
...
impl<R: Read, P: PartProvider<R>> StreamingEntries<R, P> {
    ...
    pub fn next_entry(&mut self) -> io::Result<Option<StreamingReadEntry<'_, R, P>>> {
        match self.state {
            ...
            state => return Err(cursor_state_error(state)),
        }
...
impl<C: ChunkCursor> DecodedEntryReaderCore<'_, C> {
    /// Returns the entry header while streaming the payload.
    #[inline]
    const fn header(&self) -> &EntryHeader {
        &self.header
    }
    ...
    fn finish(mut self) -> io::Result<EntryCompletion> {
        ...
        self.completion.take().ok_or_else(|| {
            io::Error::new(io::ErrorKind::InvalidData, "entry did not reach completion")
        })
    }
    ...
    fn load_next(&mut self) -> io::Result<()> {
        ...
        match chunk.ty() {
            ChunkType::FDAT => {
                ...
                self.encoded_size = checked_size_add(
                    self.encoded_size,
...
impl<'a, R: Read, P: PartProvider<R>> EncodedSolidReader<'a, R, P> {
    ...
    fn finish(self) -> io::Result<()> {
        ...
        let _: SolidEntry = RawEntry(self.chunks).try_into()?;
        Ok(())
    }
}
...
fn checked_size_add(current: u64, added: u64, name: &str) -> io::Result<u64> {
    current
        .checked_add(added)
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, format!("{name} overflow")))
}
